...
export async function fetchEscrowEvents(fromBlock: bigint, toBlock: bigint) {
  const address = env.ESCROW_CONTRACT_ADDRESS as `0x${string}`;

  const [created, claimed, cancelled, expired] = await Promise.all([
    client.getLogs({ address, event: bountyCreatedEvent, fromBlock, toBlock }),
    client.getLogs({ address, event: bountyClaimedEvent, fromBlock, toBlock }),
    client.getLogs({ address, event: bountyCancelledEvent, fromBlock, toBlock }),
    client.getLogs({ address, event: bountyExpiredEvent, fromBlock, toBlock })
  ]);
